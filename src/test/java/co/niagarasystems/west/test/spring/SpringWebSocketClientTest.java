...
import co.niagarasystems.west.client.spring.SpringWestClient;
import co.niagarasystems.west.io.CodecUtils;
import co.niagarasystems.west.test.Account;
import co.niagarasystems.west.test.Person;
...
public class SpringWebSocketClientTest {
    ...
    private static final Logger log =
        Logger.getLogger(SpringWebSocketClientTest.class);
        ...
        try {
            ...
            applicationContext =
                new ClassPathXmlApplicationContext(
                    new String[] {
                        "classpath:META-INF/spring/test/appContext2.xml"
                    });
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    @Before
    public void init() {
        client = applicationContext.getBean("springWestClient", IWestClient.class);
    ...
    public static void cleanup() {
        ...
        try {
            IWestClient client = applicationContext.getBean("springWestClient", IWestClient.class);

            // Close session
            ((SpringWestClient) client).getSession().close();
